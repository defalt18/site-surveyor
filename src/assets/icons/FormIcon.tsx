import * as React from 'react';
import { SVGProps } from 'react';

const SvgComponent = (props: SVGProps<any>) => (
	<svg
		width={73}
		height={73}
		fill='none'
		xmlns='http://www.w3.org/2000/svg'
		xmlnsXlink='http://www.w3.org/1999/xlink'
		{...props}
	>
		<path opacity={0.1} fill='url(#form_a)' d='M.998.012h72v72h-72z' />
		<defs>
			<pattern id='form_a' patternContentUnits='objectBoundingBox' width={1} height={1}>
				<use xlinkHref='#form_b' transform='scale(.01042)' />
			</pattern>
			<image
				id='form_b'
				width={96}
				height={96}
				xlinkHref='data:image/png;base64,'
			/>
		</defs>
	</svg>
);

export default SvgComponent;
