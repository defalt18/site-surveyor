	...
	return (
		<div className='flex flex-col gap-y-[1.4rem]'>
			{map(keys(FEATURES), (key) => (
				<div key={key} className={c('bg-light-primary rounded-lg px-[1.2rem] py-[2.9rem]')}>
					<p className='text-primary mb-[2.6rem] text-dark-primary'>{key}</p>
					<div className='grid grid-cols-2 gap-x-[0.9rem] gap-y-[0.8rem]'>
						{map(FEATURES[key].data, (item, index) => {
